...
func IOCounters(names ...string) (map[string]IOCountersStat, error) {
	...
	if inZone {
		ret := make(map[string]IOCountersStat, 1)
		ioStat, err := IOCountersZone()
		if err != nil {
			return nil, err
		}

		ret[ioStat.Name] = IOCountersStat{
			ReadCount:        ioStat.NumReadOps,
			MergedReadCount:  0,
			WriteCount:       ioStat.NumWriteOps,
			MergedWriteCount: 0,
			ReadBytes:        ioStat.BytesRead,
			WriteBytes:       ioStat.BytesWritten,
			ReadTime:         0,
			WriteTime:        0,
			IopsInProgress:   ioStat.NumRunningIO + ioStat.NumQueuedIO,
			IoTime:           uint64((ioStat.TimeIORunning + ioStat.TimeIOQueued) / time.Millisecond),
			WeightedIO:       0,
			Name:             ioStat.Name,
			SerialNumber:     ioStat.Name,
		}
...
// IOCountersZone returns the IOPS being used by the visible zone.
func IOCountersZone() (*IOCountersZoneStat, error) {
	kstatPath, err := common.KStatPath()
	if err != nil {
		return nil, fmt.Errorf("cannot find kstat(1M): %v", err)
	}

	kstatJSON, err := invoke.Command(kstatPath, "-jm", "zone_vfs")
	if err != nil {
	...
	var ret *IOCountersZoneStat
	for _, zoneStat := range kstatFrames {
		ret = &zoneStat.Data

		// Fixup DelayTime: kstat(1M) reports this value in µs but the value is
		// Unmarshaled as ns
		ret.DelayTime = ret.DelayTime * time.Microsecond
