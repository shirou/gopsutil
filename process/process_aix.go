...
func (p *Process) SignalsPendingWithContext(ctx context.Context) (SignalInfoStat, error) {
	...
	// Read the status file directly to get the SigPend field
	psInfoPath := common.HostProcWithContext(ctx, strconv.Itoa(int(p.Pid)), "status")
...
func (p *Process) fillFromExeWithContext(ctx context.Context) (string, error) {
	...
	// Prefer address space read — returns full untruncated argv[0]
	args, err := p.readArgsFromAddressSpace(ctx, int(pid), &aixPSinfo, 1)
	if err == nil && len(args) > 0 && isValidProcessName(args[0]) {
		return filepath.Base(args[0]), nil
	}

	// Fallback to Fname (limited to 16 bytes, may truncate long names)
	fname := extractFnameString(&aixPSinfo)
	if fname != "" {
		return fname, nil
	}

	// Last resort: extract basename from Psargs (limited to 80 bytes)
...
func (p *Process) readArgsFromAddressSpace(ctx context.Context, pid int, psinfo *AIXPSInfo, maxArgs int) ([]string, error) {
	...
	// On AIX, psinfo.Argv is a pointer TO the argv array (char ***),
	// not the argv array itself. Dereference once to get the real argv base.
	argvPtrBuf := make([]byte, ptrSize)
	n, err := syscall.Pread(fd, argvPtrBuf, int64(psinfo.Argv))
	if err != nil || n != len(argvPtrBuf) {
		return nil, err
	}
	var argvBase int64
	if ptrSize == 8 {
		argvBase = int64(binary.BigEndian.Uint64(argvPtrBuf))
	} else {
		argvBase = int64(binary.BigEndian.Uint32(argvPtrBuf))
	}

	// Read argv pointers from the real argv array
	...
	for i := 0; i < argc; i++ {
		offset := argvBase + int64(i)*ptrSize
...
func (p *Process) fillFromCmdlineWithContext(ctx context.Context) (string, error) {
	...
	// Prefer address space read — returns full untruncated arguments
	args, err := p.readArgsFromAddressSpace(ctx, int(pid), &aixPSinfo, 0)
	if err == nil && len(args) > 0 && isValidProcessName(args[0]) {
		return strings.Join(args, " "), nil
	}

	// Fallback to Psargs (limited to 80 bytes, may truncate)
	psargs := extractPsargsString(&aixPSinfo)
	if psargs != "" {
		return psargs, nil
	}
...
func (p *Process) fillSliceFromCmdlineWithContext(ctx context.Context) ([]string, error) {
	...
	// Prefer address space read — returns full untruncated arguments as separate strings
	args, err := p.readArgsFromAddressSpace(ctx, int(pid), &aixPSinfo, 0)
	if err == nil && len(args) > 0 && isValidProcessName(args[0]) {
		return args, nil
	}

	// Fallback to Psargs (limited to 80 bytes, splits on spaces which may be inaccurate)
	psargs := extractPsargsString(&aixPSinfo)
	if psargs != "" {
...
// isValidProcessName checks if a string contains only printable ASCII characters
// suitable for a process name or path.
func isValidProcessName(s string) bool {
...
func (p *Process) fillFromTIDStatWithContext(ctx context.Context, tid int32) (uint64, int32, *cpu.TimesStat, int64, uint32, int32, *PageFaultsStat, error) {
	...
	infoPath = common.HostProcWithContext(ctx, strconv.Itoa(int(pid)), "psinfo")
	if tid > -1 {
		// Search for lwpstatus and lwpsinfo files, handling unknown directory structures
	...
	createTime := int64(startTime)*1000 + int64(aixPSinfo.Start.TvNsec)/1000000
...
// findLwpFiles searches recursively for lwpstatus and lwpsinfo files under a given directory.
...
func findLwpFiles(basePath string) (string, string) {
	...
	directInfoPath := filepath.Join(basePath, "lwpsinfo")
...
// searchLwpFilesRecursive recursively searches for lwpstatus and lwpsinfo files.
