...
// WillBeDeleteOptOutMemAvailableCalc is a context key to opt out of calculating Mem.Used.
...
const WillBeDeleteOptOutMemAvailableCalc = "optOutMemAvailableCalc"
...
func fillFromMeminfoWithContext(ctx context.Context) (*VirtualMemoryStat, *ExVirtualMemory, error) {
	...
	// see https://github.com/shirou/gopsutil/issues/1873
	if val, ok := ctx.Value(WillBeDeleteOptOutMemAvailableCalc).(bool); ok && val {
		if !memavail {
